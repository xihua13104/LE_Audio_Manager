...
static lm_status_t lm_unregister_agent(lm_agent_t *agent);
static lm_status_t lm_agentmanager_register_agent(lm_agent_t *agent) ;
static lm_status_t lm_agentmanager_unregister_agent(lm_agent_t *agent);
...
void lm_agent_destroy(lm_agent_t *agent) {
    ...
    lm_unregister_agent(agent);
    lm_agentmanager_unregister_agent(agent);
                                ...
                                void *userdata)
{
    ...
    if (g_str_equal(method, AGENT_METHOD_REQUEST_PIN_CODE)) {
        ...
        status = lm_app_event_callback(LM_AGENT_REQ_PASSKEY_IND, LM_STATUS_SUCCESS, &ind);
        if (status != LM_STATUS_SUCCESS) {
            g_dbus_method_invocation_return_dbus_error(invocation,
                            "org.bluez.Error.Rejected", "No passkey inputted");
            return;
        }
...
static lm_status_t lm_unregister_agent(lm_agent_t *agent) {
    if (agent->registration_id) {
        g_dbus_connection_unregister_object(agent->dbus_conn, agent->registration_id);
...
static lm_status_t lm_agentmanager_unregister_agent(lm_agent_t *agent) {
    g_assert(agent != NULL);
    return lm_agentmanager_call_method(agent->dbus_conn,
            AGENT_MANAGER_METHOD_UNREGISTER, g_variant_new("(o)", agent->path));
}
