...
struct lm_transport {
    ...
    lm_transport_audio_location_t location; /* Indicates transport Audio Location. */
    guint8 *meta;
    gsize meta_size;
...
lm_transport_t *lm_transport_create(lm_device_t *device, const gchar *path)
{
    ...
    memset(&transport->qos.bcode, 0, LM_TRANSPORT_BCAST_CODE_SIZE);
                                        ...
                                        gpointer user_data)
{
    ...
    if (error != NULL) {
        lm_log_error(TAG, "failed to call transport method (error %d '%s')", error->code, error->message);
        g_clear_error(&error);
    }
}
...
static lm_status_t __attribute__((unused)) lm_transport_set_property(lm_transport_t *transport, const gchar *property, GVariant *value)
{
    g_assert(transport != NULL);
...
lm_status_t lm_transport_set_links(GPtrArray *transports)
{
    ...
    if (transports->len < 2) {
       lm_log_warn(TAG, "No need to link transports");
       return LM_STATUS_SUCCESS;
    }
    ...
    lm_transport_set_property((lm_transport_t *)g_ptr_array_index(transports, 0),
...
lm_status_t lm_transport_set_volume_percentage(lm_transport_t *transport, float volume_per)
{
    ...
    return lm_transport_set_property(transport, MEDIA_TRANSPORT_PROPERTY_VOLUME,
                                            ...
                                            lm_transport_bcast_code_t *bcode)
{
    ...
    GVariant *bcode_array = g_variant_new_fixed_array(G_VARIANT_TYPE_BYTE,
                                  bcode->code,
                                  sizeof(lm_transport_bcast_code_t),
                                  sizeof(guint8));
    ...
    GVariant *qos_dict = g_variant_builder_end(&builder);

    return lm_transport_set_property(
                transport,
                MEDIA_TRANSPORT_PROPERTY_QOS,
                qos_dict);
}
                  ...
                  const gchar *property_name, GVariant *property_value)
{
    ...
    if (g_str_equal(property_name, MEDIA_TRANSPORT_PROPERTY_DEVICE)) {
        ...
        transport->config = g_memdup(config, size);
        transport->config_size = size;
        lm_log_debug(TAG, "transport config:");
        lm_log_hex_dump(LM_LOG_DEBUG, TAG, transport->config, transport->config_size);
        ...
        transport->location = (lm_transport_audio_location_t)g_variant_get_uint32(property_value);
        ...
        transport->meta = g_memdup(metadata, size);
        transport->meta_size = size;
        lm_log_debug(TAG, "transport meta:");
        lm_log_hex_dump(LM_LOG_DEBUG, TAG, transport->meta, transport->meta_size);
        ...
        while (g_variant_iter_loop(&qos_iter, "{sv}", &qos_key, &qos_value)) {
            ...
            } else if (g_str_equal(qos_key, "SyncFactor")) {
                transport->qos.sync_factor = g_variant_get_byte(qos_value);
                lm_log_debug(TAG, "sync factor 0x%x", transport->qos.sync_factor);
            } else if (g_str_equal(qos_key, "Packing")) {
                transport->qos.packing = g_variant_get_byte(qos_value);
                lm_log_debug(TAG, "packing 0x%x", transport->qos.packing);
            } else if (g_str_equal(qos_key, "Skip")) {
                transport->qos.skip = g_variant_get_uint16(qos_value);
                lm_log_debug(TAG, "skip 0x%x", transport->qos.skip);
            } else if (g_str_equal(qos_key, "SyncTimeout")) {
                transport->qos.sync_timeout = g_variant_get_uint16(qos_value);
                lm_log_debug(TAG, "sync timeout 0x%x", transport->qos.sync_timeout);
            } else if (g_str_equal(qos_key, "SyncType")) {
                transport->qos.sync_type = g_variant_get_byte(qos_value);
                lm_log_debug(TAG, "sync type 0x%x", transport->qos.sync_type);
            } else if (g_str_equal(qos_key, "MSE")) {
                transport->qos.mse = g_variant_get_byte(qos_value);
                lm_log_debug(TAG, "mse 0x%x", transport->qos.mse);
            } else if (g_str_equal(qos_key, "Timeout")) {
                transport->qos.timeout = g_variant_get_uint16(qos_value);
                lm_log_debug(TAG, "timeout 0x%x", transport->qos.timeout);
            } else if (g_str_equal(qos_key, "BCode")) {
                gsize bcode_size;
                const guint8 *bcode = g_variant_get_fixed_array(qos_value, &bcode_size, sizeof(guint8));
                memset(&transport->qos.bcode, 0, LM_TRANSPORT_BCAST_CODE_SIZE);
                memcpy(&transport->qos.bcode, bcode, bcode_size);
                lm_log_info(TAG, "transport bcode:", (gchar *)transport->qos.bcode.code);
                lm_log_hex_dump(LM_LOG_INFO, TAG, transport->qos.bcode.code, bcode_size);
            } else if (g_str_equal(qos_key, "Encryption")) {
                transport->qos.encryption = g_variant_get_byte(qos_value);
                lm_log_info(TAG, "encryption %s", transport->qos.encryption ? "true" : "false");
