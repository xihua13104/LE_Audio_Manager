...
static void lm_player_track_destroy(lm_player_track_t *track)
{
    ...
    if (track->general_name)
        g_free((gpointer)track->general_name);
...
{
    lm_log_debug(TAG, "player '%s' property update",  player->path, property_name);
    if (g_str_equal(property_name, MEDIA_PLAYER_PROPERTY_DEVICE)) {
        ...
        while (g_variant_iter_loop(&track_iter, "{sv}", &track_key, &track_value)) {
            if (g_str_equal(track_key, "Title")) {
                ...
                if (player->track->general_name)
                    g_free((gpointer)player->track->general_name);
                player->track->general_name = g_strdup(g_variant_get_string(track_value, NULL));
                lm_log_debug(TAG, "general name '%s'", player->track->general_name);
