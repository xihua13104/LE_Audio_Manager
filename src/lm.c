...
{
    guint32 i = 0;
    lm_status_t result = LM_STATUS_FAIL;
    ...
    for (i = 0; i < LM_APP_CALLBACK_MAX; i++) {
        if (app_callback_table[i].in_use && app_callback_table[i].cb && app_callback_table[i].mask & module_mask) {
            result = app_callback_table[i].cb(msg, status, buf);
            if (result != LM_STATUS_SUCCESS ) {
                lm_log_error(TAG, "app callback failed for msg:0x%08x, module mask:0x%08x", msg, module_mask);
                return result;
            }
        }
    }
